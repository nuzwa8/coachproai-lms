...
define('NUZ_ACADEMY_VERSION', '1.0.0');
...
class NUZ_Online_Academy {
    ...
    public function admin_scripts($hook) {
        ...
        $is_dashboard = isset($_GET['page']) && $_GET['page'] === 'nuz-academy';
    ...
    private function get_dashboard_stats() {
        ...
        // Get counts
        $total_students = $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}nuz_students WHERE status = 'active'");
        $total_courses = $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}nuz_courses WHERE status = 'active'");
        $total_payments = $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}nuz_payments");
        $total_revenue = $wpdb->get_var("SELECT COALESCE(SUM(amount), 0) FROM {$wpdb->prefix}nuz_payments WHERE payment_status = 'paid'");
        $pending_fees = $wpdb->get_var("SELECT COALESCE(SUM(amount - COALESCE(paid_amount, 0)), 0) FROM {$wpdb->prefix}nuz_payments WHERE payment_status != 'paid'");
        
        $stats = array(
            'total_students' => intval($total_students),
            'total_courses' => intval($total_courses),
            'total_revenue' => floatval($total_revenue),
            'pending_fees' => floatval($pending_fees)
        );
